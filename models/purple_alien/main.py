...
def parse_args():
    ...
    parser.add_argument('-s', '--sweep',
                        ...
                        help='Choose whether to run the model pipeline as part of a sweep. Default is False. '
                             'Note: If --sweep is True, --run_type must be calibration, and both --train and --evaluate will be set to True.')
...
def validate_arguments(args):
    if args.sweep:
        if args.run_type != 'calibration':
            ...
            print("To fix: Use --run_type calibration when --sweep is specified.")
            sys.exit(1)
        args.train = True
        args.evaluate = True
