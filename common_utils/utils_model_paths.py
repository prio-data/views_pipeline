...
class ModelPath:
    def __init__(self, model_name: str, validate=True) -> None:
        """
        ...
        self._common_querysets = self.root / "common_querysets"
        if self._common_querysets not in sys.path:
            sys.path.insert(0, str(self._common_querysets))
        self._queryset_path = self._common_querysets / f"queryset_{self.model_name}.py"
        self._queryset = None
    ...
    def _get_model_dir(self) -> Path:
        ...
        if self.validate and not self._check_if_dir_exists(model_dir):
            raise FileNotFoundError(
        ...
        if self._sys_paths is None:
            ...
            for attr, value in self.__dict__.items():
                if attr not in [
                    ...
                    # Add path only if it is a Path object, is absolute, and exists
                    if (
            ...
            try:
                sys.path.extend(self._sys_paths)
            except AttributeError:
                logger.exception("sys.path not found. Please import sys and try again.")
        else:
            logger.error(
                "Paths already added to sys.path. Remove them by calling remove_paths_from_sys()."
            )
