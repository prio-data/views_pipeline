...
sys.path.insert(0, str(Path(
    *[i for i in PATH.parts[:PATH.parts.index("views_pipeline") + 1]]) / "common_utils")) 
...
def create_log_file(PATH_GENERATED,
    ...
    if models:
        for m_name in models:
            model_path = ModelPath(m_name, validate=False)
            log_data = read_log_file(model_path.data_generated / f"{run_type}_log.txt")
