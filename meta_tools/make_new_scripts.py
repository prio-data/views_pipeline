...
class ModelScriptBuilder:
    ...
    def build(self):
        ...
        template_config_hyperparameters.generate(
            script_dir=self.model_dir / "configs/config_hyperparameters.py", model_architecture=self.model_architecture)
        template_config_input_data.generate(
            script_dir=self.model_dir / "configs/config_input_data.py", model_name=self.model_name)
        template_config_meta.generate(
            script_dir=self.model_dir / "configs/config_meta.py", model_name=self.model_name, model_architecture=self.model_architecture)
        template_config_sweep.generate(
            script_dir=self.model_dir / "configs/config_sweep.py", model_architecture=self.model_architecture)
        # For later, works.
